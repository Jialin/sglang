...
def _model_runner(*, spec_none=True, compliant=True):
...
def test_gates_exclude_non_plain_prefill_and_noncompliant():
    with envs.SGLANG_ENABLE_PREFILL_WAR_READ_DONE.override(True):
        for runner, batch in (
            ...
            # Spec configs are fenced by the draft-extend publish.
            (_model_runner(spec_none=False), _batch()),
